import { ModeInfo } from "@/types/chat";

interface ModeCardProps {
  mode: ModeInfo;
  onClick: () => void;
}

export default function ModeCard({ mode, onClick }: ModeCardProps) {
  return (
    <button
      onClick={onClick}
      className="w-full h-full p-4 bg-card border border-border rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 text-left active:scale-95"
    >
      <div className="flex flex-col items-center text-center gap-3">
        <div className="text-5xl">{mode.icon}</div>
        <div className="space-y-1">
          <h3 className="text-base font-semibold text-card-foreground leading-tight">
            {mode.title}
          </h3>
          <p className="text-xs text-muted-foreground leading-relaxed line-clamp-2">
            {mode.subtitle}
          </p>
        </div>
      </div>
    </button>
  );
}
